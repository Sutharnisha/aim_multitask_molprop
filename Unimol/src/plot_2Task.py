"""
Uni-Mol comparison plots for the original QM9 3-task run (mu, U0, U).

This plots the legacy 3-task result directories (results_lr5e4_S_*),
trained before the pilot switched to the 2-task (mu, eps_LUMO) setup used
by plot_comparison.py / data.py. Kept separate so the old mu/U0/U runs stay
plottable.
...
RESULTS_DIR_CANDIDATES = [
    Path("../results/results_lr5e4_S_42"),
]
...
fig.suptitle(
    f"Uni-Mol n_train={num} Seed={seed} · QM9 3-task (mu, U0, U)",
...
plot_bar_chart(ax_bar, bests, stl_test, f"Uni-Mol n_train={num} Seed={seed}")
