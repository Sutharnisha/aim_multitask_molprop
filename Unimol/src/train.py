...
For each epoch:
  ...
  [POLICY UPDATE  — on guidance batch]
    6. Compute g_i_guide on guidance batch (detached → no graph through model)
...
For baselines (ls, pcgrad):
  Steps 1-2 same; step 3 uses LS/PCGrad combiner; steps 6-9 skipped.
...
Training protocol notes:
    ...
    - Dataset splits and loaders are unchanged.
"""
...
from baselines import combine_gradients, BASELINE_METHODS
from metrics import print_results_table

BASELINE_METHODS = list(BASELINE_METHODS) + ["stl"]
...
def _fill_head_grads_and_override_encoder(
    ...
    """
    One forward+backward on the mean loss to fill head gradients,
    ...
    loss.backward()                                             # fills head grads
...
# ---------------------------------------------------------------------------
# LS step (optimised: one backward, no per-task split needed)
# ---------------------------------------------------------------------------
...
def _step_ls(
    ...
    """Linear scalarization: backward on mean loss."""
    batch = batch_to_device(batch, device)
    targets_norm = (batch["targets"] - means) / stds
...
# ---------------------------------------------------------------------------
# PCGrad step (N sequential forward passes)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AIM step (N sequential forward passes + policy update)
# ---------------------------------------------------------------------------

def _step_aim(
    model:            UniMolMultiTask,
    policy:           torch.nn.Module,       # lives on CPU
...
def evaluate(
    ...
    for batch in loader:
        batch   = batch_to_device(batch, device)
        targets = batch["targets"]                  # [B, 3]  physical units
        B       = targets.shape[0]
        ...
        preds = preds_norm * stds + means           # [B, 11]  physical units
...
def train(
    ...
    # Only add trainable params to optimizer (saves Adam state memory for frozen layers)
    ...
    if method == "aim_scalar":
        ...
        raise ValueError(f"Unknown method: {method!r}. "
                         f"Choose from: ls, pcgrad, aim_scalar, aim_matrix")
