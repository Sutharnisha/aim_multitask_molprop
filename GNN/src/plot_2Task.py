"""
GNN comparison plots for the current QM9 2-task pilot.

Produces comparison_all_n5000.png with:
  Row 1 : validation MAE curves for mu and eps_LUMO
...
def build_stl_test(base: Path) -> dict:
    """Return the available STL reference values for the current task set."""
    stl_run = resolve_stl_run(base)
    if not stl_run:
        return {"mu": np.nan, "eps_LUMO": np.nan}
    stl_vals = {"mu": best_stl_test(stl_run, "mu", base)}
    stl_vals["eps_LUMO"] = np.nan
    return stl_vals
...
fig.suptitle(
    f"GNN n_train={num} Seed={seed} · QM9 2-task pilot",
...
ax_mu = fig.add_subplot(gs[0, 0:3])
ax_eps = fig.add_subplot(gs[0, 3:6])
ax_bar = fig.add_subplot(gs[1, 0:4])
ax_heat = fig.add_subplot(gs[1, 4:6])

task_axes = {"mu": ax_mu, "eps_LUMO": ax_eps}
...
def plot_bar_chart(ax, bests_dict, stl_vals, title_prefix):
    ...
    ax.set_title(f"{title_prefix} — Best Test MAE: All Methods vs Both Tasks", fontsize=10)
...
plot_bar_chart(ax_bar, bests, stl_test, "GNN")


def plot_tau_heatmap(ax, bests_dict):
    best_aim = bests_dict["AIM Matrix"]
    tau_matrix = np.array(best_aim["tau"], dtype=float)
    if tau_matrix.ndim == 0 or tau_matrix.shape != (2, 2):
        tau_matrix = np.full((2, 2), float(tau_matrix.flat[0]))
...
header = (
    f"  {'Method':<13}  {'GNN Dm%(mu)':>12}  |"
    f"  {'mu':>7}  {'eps_LUMO':>10}"
)
sep = "  " + "-" * (len(header) - 2)

rows = [
    "Positive Δm%(mu) means better than the STL baseline for mu:",
    header,
    sep,
]
for lbl in RUNS:
    gnn_dm = delta_m_pct(bests[lbl]["test_per_task"], stl_test)
    gnn_t = bests[lbl]["test_per_task"]
    rows.append(
        f"  {lbl:<13}  {gnn_dm:>+11.2f}%  |"
        f"  {gnn_t['mu']:>7.3f}  {gnn_t['eps_LUMO']:>10.3f}"
    )
rows.append(sep)
rows.append(
    f"  {'STL Ref':<13}  {'0.00%':>12}  |"
    f"  {stl_test['mu']:>7.3f}  {'n/a':>10}"
)
